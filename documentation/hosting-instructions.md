...
- ssh into the server
    ...
    - For Windows: Set up a new PuTTY session
        ...
        - **Session**
            ...
            - Click Save
- create user
    - `useradd -m -s /bin/bash -g users -G sudo krempen`
    - replace gkephart with your own
- create password
    - `passwd krempen`
- Move .ssh key file
    - `cp -r .ssh /home/krempen`
- change ownership of the .ssh directory
    - `cd /home/krempen`
    - `chown -R krempen:users .ssh/`


- Test Changes
    - `exit`
    - Mac/Linux: `ssh krempen@kilianrempen.com`
    - Windows:
        - **Session**
            - Click pwp
            - Click Load
...
- reboot
    - `sudo reboot`
- ssh back in
- `ssh krempen@kilianrempen.com (or IP address)`
...
- create a directory for your deployment
    - `sudo mkdir /var/www/kilianrempen.com/html`
- fix permissions
    - `sudo chown -R $USER:users /var/www/kilianrempen.com/html`
...
- `sudo vim /etc/nginx/sites-available/kilianrempen.com`
...
- create a symlink to the sites-enabled directory
    - `sudo ln -s /etc/nginx/sites-available/kilianrempen.com /etc/nginx/sites-enabled/`
...
- Create a new ssh key on your server in the .ssh directory
    - `ssh-keygen -t ed25519 -C  "github Deploy Key" -f /home/krempen/.ssh/github_deploy_key`
    - Don't enter a passphrase
        - hit enter twice
- Copy the public key into your authorized keys file
    - `cat /home/krempen/.ssh/github_deploy_key.pub >> /home/krempen/.ssh/authorized_keys`
...
- Copy the private key to your clipboard **(this is a very dangerous operation do not share this key with anyone )**
    ...
    - `cat /home/krempen/.ssh/github_deploy_key`
  ...
  deploy:
    ...
    steps:
      ...
      - name: Deploy to server
        run: |
         ssh-keyscan -H kilianrempen.com >> ~/.ssh/known_hosts 
         scp -r ./dist/* krempen@kilianrempen.com:/var/www/kilianrempen.com/html
```
